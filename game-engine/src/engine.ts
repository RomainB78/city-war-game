...
export function createInitialState(
  ...
  for (const [key, config] of Object.entries(INITIAL_CITIES_CONFIG)) {
...
export function validateExchangeAction(
  ...
  const neighbors = ADJACENCY_MAP[sourceCityId] || [];
...
// Relocate Capital
export function validateRelocateCapital(
  state: GameState,
  playerFaction: Faction,
  args: RelocateCapitalArgs
): { valid: boolean; error?: string } {
...
function checkVictory(state: GameState): Faction | null {
...
export function resolveCombat(state: GameState): GameState {
  ...
  // Run simulation
  const result = simulateCombat(attackerBastion.soldiers, defenderBastion.soldiers);
...
export function clearCombatState(state: GameState): GameState {
  const newState = JSON.parse(JSON.stringify(state)) as GameState;
